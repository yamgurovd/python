...
Link - https://stepik.org/lesson/265082/step/9?auth=registration&unit=246030
"""

color = input("Введите 2 цвета через пробел: ").split()

violet: list[str] = ['red', 'blue']
orange: list[str] = ['red', 'yellow']
green: list[str] = ['blue', 'yellow']

if color in violet and len(color) == len(violet):
    print("фиолетовый")
elif  color in orange and len(color) == len(orange):
    print("оранжевый")
elif  color in green and len(color) == len(green):
    print("зеленый")
else:
    print("ошибка цвета")

print(len(color), len(violet), len(orange), len(green))
assert color == violet and len(color) == len(violet)
